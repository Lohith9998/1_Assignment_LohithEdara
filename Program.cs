    ...
    class Program
    {
        ...
        private static string RemoveVowels(string s)
        {
            try
            {
                ...
                { 
                    ...
                    {
                        final_string = final_string + le; // after removing vowels and add remaining
                    }
                }
                return final_string; // return the output
            }
            catch (Exception)
            {
                throw;
       ...
       */
        ...
        private static bool ArrayStringsAreEqual(string[] bulls_string1, string[] bulls_string2)
        {
            ...
            {
                // write your code here.
                if (string.Join("", bulls_string1) == string.Join("", bulls_string2)) // Using Joining method and Condition - If both strings are equal it returns true or else it is false
                {
                    return true; // if condition satisfies then true
                }
                else
                {
                    return false; // if condition not satisfies then false
                }
            }
            catch (Exception)
            {
        ...
        */
        ...
        private static int SumOfUnique(int[] bull_bucks)
        {
            try
            {
                ...
                {
                    int num = 0;
                    for (int i = 0; i < le; i++) // // for loop and incrementing
                    {
                        if (e == bull_bucks[i]) // // if case 
                        {
                            num = num + 1; // // adding one 
                        }
                    }
                    if (num == 1) // if it is equivalent to 1
                    {
        ...
        */
        ...
        private static int DiagonalSum(int[,] bulls_grid)
        {
            try
            {
                ...
                if (se % 2 != 0)
                {
                    return ab - bulls_grid[(se - 1) / 2, (se - 1) / 2];
                }
                else
                {
                    return ab;
        ...
        */
        ...
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            try
            {
                ...
                string le = "";
                for (int jk = sf[0].Length - 1; jk >= 0; jk--)
                {
                    le = le + bulls_string6[jk];
                }
                String prefix_string = ch + le + sf[1];
